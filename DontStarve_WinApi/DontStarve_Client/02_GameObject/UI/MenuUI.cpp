...
void MenuUI::CreateMenuBar()
{
    ...
    AddManaged(objMgr->CreateButton(GOID_UI_BUTTON, m_iconSize, m_iconSize, L"Resource/UI/CraftIcon.png", L"Resource/UI/CraftIcon.png", 0.0f, 0.5f, iconX, m_menuIconY[4], [this]() { ToggleToolList(); }));
    AddManaged(objMgr->CreateButton(GOID_UI_BUTTON, m_iconSize + 20, m_iconSize + 20, L"Resource/UI/CreateIcon.png", L"Resource/UI/CreateIcon.png", 0.0f, 0.5f, iconX, m_menuIconY[3], [this]() { ToggleCreateList(); }));
    AddManaged(objMgr->CreateButton(GOID_UI_BUTTON, m_iconSize, m_iconSize, L"Resource/UI/CookIcon.png", L"Resource/UI/CookIcon.png", 0.0f, 0.5f, iconX, m_menuIconY[2], [this]() { ToggleCookList(); }));
    AddManaged(objMgr->CreateButton(GOID_UI_BUTTON, m_iconSize, m_iconSize, L"Resource/UI/BattleIcon.png", L"Resource/UI/BattleIcon.png", 0.0f, 0.5f, iconX, m_menuIconY[1], [this]() { ToggleBossPanel(); }));
    AddManaged(objMgr->CreateButton(GOID_UI_BUTTON, m_iconSize, m_iconSize, L"Resource/UI/EditIcon.png", L"Resource/UI/EditIcon.png", 0.0f, 0.5f, iconX, m_menuIconY[0], []() {}));

    AddManaged(objMgr->CreateImage(GOID_UI_MENU, m_craftBarWidth, m_craftBarHeight, LAYER_UI_BACKGROUND, L"Resource/UI/CraftBar.png", 0.0f, 0.0f, 0.5f, 50.0f, 0.0f));
}
...
void MenuUI::CreatePalettes()
{
    ...
    auto CreateGrid = [&](const std::vector<GameObjectID>& items, std::vector<GameObject*>& group) {
        for (size_t i = 0; i < items.size(); ++i) {
            ...
            UIButton* btn = AddManaged(objMgr->CreateButton(GOID_UI_BUTTON, m_toolButtonSize, m_toolButtonSize, path, path, 0.0f, 0.5f, x, y, [this, id = items[i]]() { SelectTool(id); }));
            if (btn) {
    ...
    m_toolPanelBg = AddManaged(objMgr->CreateImage(GOID_UI_IMAGE, toolPanelW, toolPanelH, LAYER_UI_BACKGROUND, L"Resource/UI/CraftUI_PaletteBG.png", -0.05f, 0.0f, 0.5f, toolPanelCX + 20.0f, toolPanelCY - 30.0f));
}
...
void MenuUI::CreateIngredientUI()
{
    ...
    m_craftButton = AddManaged(objMgr->CreateButton(GOID_UI_BUTTON, 120.0f, 40.0f, L"Resource/UI/frontscreen.png", L"Resource/UI/HighLight_frontscreen.png", 0.0f, 0.5f, m_ingredientPanelCenterX, craftButtonY, [this]() {
        Player* player = ObjectManager::GetInstance()->GetPlayer();
        if (player) TryCraftSelectedTool(player);
    }));
    m_craftButtonText = AddManaged(objMgr->CreateText(GOID_UI_TEXT, 120.0f, 40.0f, L"제작하기", Gdiplus::Color::Black, 14.0f, 0.0f, 0.5f, m_ingredientPanelCenterX, craftButtonY));
...
{
    auto* objMgr = ObjectManager::GetInstance();
    m_bossOverlay = AddManaged(objMgr->CreateImage(GOID_UI_IMAGE, 1920.0f, 1080.0f, LAYER_UI_BACKGROUND, L"Resource/UI/frontscreen.png", -0.1f, 0.5f, 0.5f, 0.0f, 0.0f));
    ...
    auto SetupBossPanel = [&](int idx, GameObjectID bossID, const std::wstring& spritePath, float x) {
        ...
        m_bossPanels[idx].clearText = AddManaged(objMgr->CreateText(GOID_UI_TEXT, 200.0f, 60.0f, L"CLEAR", Gdiplus::Color::Red, 48.0f, 0.5f, 0.5f, x, 0.0f));
    ...
    m_bossChallengeButtonText = AddManaged(objMgr->CreateText(GOID_UI_TEXT, 200.0f, 60.0f, L"보스 도전", Gdiplus::Color::Black, 18.0f, 0.5f, 0.5f, 0.0f, 280.0f));
...
void MenuUI::ToggleToolList() { bool open = (m_currentPanel != PanelType::TOOL); ClearAllPanels(); if (open) { m_currentPanel = PanelType::TOOL; for (auto* obj : m_toolGroup) obj->SetActive(true); m_toolPanelBg->SetActive(true); } }
...
{
    if (m_selectedToolID == toolID) { m_selectedToolID = GOID_NONE; m_ingredientPanelBg->SetActive(false); for (auto* obj : m_ingredientGroup) obj->SetActive(false); return; }
...
{
    if (m_selectedToolID == GOID_NONE) return;
    m_craftingItemNameText->SetText(ResourceUtils::GetResourceDisplayName(m_selectedToolID));
    
    auto* invMgr = InventoryManager::GetInstance();
    const auto* recipe = invMgr ? invMgr->GetCraftingRecipe(m_selectedToolID) : nullptr;
    int idx = 0;
    if (recipe) {
...
{
    if (m_selectedToolID == GOID_NONE || !player) return false;
    bool success = InventoryManager::GetInstance()->TryCraftItem(player, m_selectedToolID);
