...
void BerryBush::RegisterResources(ResourceManager* rm)
{
	if (!rm) return;
	GameObjectData d;
	...
	d.pivotX = 0.5f;
	d.pivotY = 1.0f;
	d.id = GOID_BERRY_TREE;
	d.objectAssetBaseDirectory = L"Resource/Objects/Bush";
	d.assetImageName = L"BerryBush.png";
	rm->RegisterObjectResource(GOID_BERRY_TREE, d);
}
