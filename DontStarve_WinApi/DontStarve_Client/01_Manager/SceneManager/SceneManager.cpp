...
{
	// 현재 씬 LateUpdate (씬에서 매니저들 LateUpdate)
	if (m_currentScene) {
...
{
	// 씬 전환 중에는 렌더링하지 않음 (초기화가 완료되지 않은 상태)
...
void SceneManager::LoadGameScene(const std::wstring& mapFileName, GameObjectID selectedCharacterID)
{
	...
	OutputDebugStringW((L"SceneManager: 게임 씬 로드 시작 - 맵: " + mapFileName +
		L", 캐릭터 ID: " + std::to_wstring(selectedCharacterID) + L", 페이드 아웃 시작\n").c_str());
}

void SceneManager::ReturnToTitle()
{
	// 타이틀 씬으로 돌아가기
	OutputDebugStringW(L"SceneManager: 타이틀 씬으로 돌아가기 시작\n");

	// 타이틀 씬 생성 (초기화는 페이드 아웃 완료 후에)
	TitleScene* titleScene = new TitleScene();
...
void SceneManager::ParseMapFileInto(const std::wstring& mapFileName, MapData& mapData)
{
	...
	while (std::getline(file, line)) {
		...
		else if (section == TILES)
		{
			...
			// 타일 데이터 파싱 - 모든 타일을 배치 (TILE_NONE도 포함)
			for (int i = 0; i < tokens.size(); i += 2)
			{
				...
				if (tileX < mapData.mapWidth && currentTileRow < mapData.mapHeight) {
					...
					if (resourceData) {
						...
						// pAtlasBitmap과 sourceRect는 나중에 로드 시점에 설정
			...
			if (std::getline(ss, type, L',') &&
				...
				// 맵 데이터에 게임오브젝트 정보만 저장 (실제 생성은 GameScene에서)
...
// 씬 타입 비교, 생성, 반환 등에서 통합 SceneType만 사용하도록 정리
...
{
	// TimeManager에서 이미 제한된 deltaTime을 받으므로 추가 제한 불필요
	...
	case TransitionState::SCENE_SWITCH:
	{
		...
		if (m_currentScene) {
			...
			if (m_currentScene->GetSceneType() == SCENE_GAME_FARMING_AREA ||
				...
				// 다른 씬들은 일반 초기화
				m_currentScene->Init();
				OutputDebugStringW(L"SceneManager: 새 씬 초기화 완료\n");
			}
		}

		// 초기화 완료 후 즉시 페이드 인 시작
		StartFadeIn();
	}
	break;
	...
	case TransitionState::FADE_IN:
		...
		if (m_fadeAlpha <= 0.0f) {
			m_fadeAlpha = 0.0f;
			m_transitionState = TransitionState::NONE;
			OutputDebugStringW(L"SceneManager: 페이드 인 완료\n");
		}
		break;
	...
	// 디버그 출력 추가 (너무 자주 출력하지 않도록 제한)
	static float debugTimer = 0.0f;
	debugTimer += deltaTime;
	if (m_transitionState != TransitionState::NONE && debugTimer >= 0.1f) {
		OutputDebugStringW((L"SceneManager: 페이드 효과 업데이트 - 알파: " + std::to_wstring(m_fadeAlpha) + L", 상태: " + std::to_wstring(static_cast<int>(m_transitionState)) + L"\n").c_str());
		debugTimer = 0.0f;
	}
}
...
void SceneManager::RenderFadeEffect()
{
	if (m_transitionState != TransitionState::NONE) {
		...
		if (renderManager) {
			// SCENE_SWITCH 상태에서는 완전히 검은 화면 유지
			BYTE alpha;
			if (m_transitionState == TransitionState::SCENE_SWITCH) {
				alpha = 255; // 완전히 검은 화면
			}
			else {
				// 알파값을 0-255 범위로 변환하고, 검은색으로 페이드
				alpha = static_cast<BYTE>(m_fadeAlpha * 255);
			}

			Gdiplus::Color fadeColor(alpha, 0, 0, 0);

			// 디버그 출력 제한 (너무 자주 출력하지 않도록)
			static float renderDebugTimer = 0.0f;
			renderDebugTimer += 0.016f; // 대략적인 프레임 시간
			if (renderDebugTimer >= 0.5f) {
				OutputDebugStringW((L"SceneManager: 페이드 효과 렌더링 - 알파: " + std::to_wstring(alpha) +
			...
			renderManager->AddFillRectangleCommand(
				...
				fadeColor,
				LAYER_DEBUG_OVERLAY,  // 가장 위에 렌더링
				999.0f  // 가장 위에 렌더링
			);
		}
		else {
			OutputDebugStringW(L"SceneManager: RenderManager가 null입니다. 페이드 효과 렌더링 실패.\n");
		}
	}
}
